...
label_filter 규칙:
- 질문이 이탈/탈퇴/삭제 리뷰에 관한 것이면 → "churn"
- 질문이 불만/컴플레인/complaint 리뷰에 관한 것이면 → "complaint"
- 질문이 긍정/만족/positive 리뷰에 관한 것이면 → "positive"
- 특정 분류를 언급하지 않으면 → null
...
- 반드시 JSON 형식으로만 응답: {"intent": ["rag"], "label_filter": "churn"} 또는 {"intent": ["sql"], "label_filter": null} 등"""
...
def orchestrate(state: AgentStateDict) -> AgentStateDict:
    ...
    # label_filter: "churn" | "complaint" | "positive" | None
    valid_labels = {"churn", "complaint", "positive"}
    label_filter = result.get("label_filter")
    if label_filter not in valid_labels:
        label_filter = None

    logger.info(f"Orchestrator 결정: intent={intent} | label_filter={label_filter}")
    return {"intent": intent, "label_filter": label_filter}
